...
func (q *query[T]) CreateIndexes(indexes []mongo.IndexModel) error {
	defer q.Close()
	log.Info("database indexing for: " + q.collection.Name())
	result, err := q.collection.Indexes().CreateMany(q.context, indexes)
	log.Info(q.collection.Name(), "result", result)
	return err
}
...
func (q *query[T]) Aggregate(pipeline mongo.Pipeline) ([]*T, error) {
	defer q.Close()
	cursor, err := q.collection.Aggregate(q.context, pipeline)
	if err != nil {
		return nil, err
	}
	defer cursor.Close(q.context)
